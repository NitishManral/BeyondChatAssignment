...
import { ChevronDown, AlignJustify, WalletCards } from 'lucide-react';
...
const InboxSidebar = ({ activeContact, onContactSelect, isMobile, isOpen, onToggle }) => {
  ...
  return (
    <>
      ...
      {/* Sidebar container */}
      <div className={`${isMobile ? 'fixed left-12 top-0 z-40 h-full' : 'relative'} w-80 bg-white flex flex-col shadow-lg transition-transform duration-300 ${isMobile && !isOpen ? '-translate-x-full' : 'translate-x-0'}`}>
            ...
            <div className="flex items-center space-x-2">
              ...
              {isMobile && (
                <button onClick={onToggle} className="lg:hidden p-1">
                  {/* X icon is missing import, needs to be added */}
        ...
        <div className="fixed bottom-0 left-0 m-3 flex items-center justify-evenly w-[70px] h-[30px] p-1 bg-white border-t border-gray-200 rounded-md shadow-lg">
          <button>
            <AlignJustify fill="black" size={18} />
          </button>
          <button>
            <WalletCards fill="black" size={18} />
          </button>
        </div>
      </div>
    </>
