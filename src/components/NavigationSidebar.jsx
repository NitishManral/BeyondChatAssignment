...
import { getUnreadCount } from '../utils/helpers'; // Utility to count unread messages
...
const NavigationSidebar = ({ isMobile, isOpen, onToggle }) => {
  ...
  // Compute total unread messages across all contacts
  ...
  // Hide sidebar completely on mobile if not open
    ...
    <>
      ...
      {isMobile && isOpen && (
        <div 
          ...
          onClick={onToggle} // Clicking overlay closes sidebar
        />
      )}
      ...
      <div className={`
        ...
        w-12 bg-gray-100 border-r border-Black flex flex-col items-center py-4
      ...
      `}>
        ...
        <div className="space-y-2">
          {bottomItems.map((item) => (
            <button
              key={item.id}
              className="w-10 h-10 rounded-lg flex items-center justify-center text-black hover:bg-Black hover:text-black transition-all duration-200 transform hover:scale-105"
            >
              <item.icon className="w-5 h-5" />
            </button>
          ))}
        ...
        <div className="mt-4 relative">
          <img
            src="https://picsum.photos/200" // Placeholder avatar
            alt="User Avatar"
